...
int countIncludes(const string a1[], int n1, const string a2[], int n2)
{
    ...
    if(n2==1 && a1[0] == a2[0]) //check if n2 ==1 and if the first elements in the array are the same, then you can start moving back and return 1;
    {
        return 1;
    ...
    if(a1[0] == a2[0])
    {
        return(countIncludes(a1+1, n1-1,a2+1, n2-1)+countIncludes(a1+1, n1-1,a2, n2)); //sum accounts for first elements matching and a1 +1 and the second array
    }
    else
    {
        return(countIncludes(a1+1, n1-1,a2, n2)); //else just shift the a1 array up 1 and recursively call countIncludes
    }
}
